...
- **Auto Play:** find and click the in-game Auto-Play button. Add an
  `auto_play.png` crop through the Image Manager first. The block keeps polling
  until the button appears; place it in Loop A or Loop B if it should retry
  independently of one-shot Battle actions.

Portal map images can be added under `Assets/map/Portal/` as
`Summer Portal.png` and `Sky Ruins Portal.png`. Select the portal family and
tier separately in the Summer task editor. Add the inventory icon crops through
Image Manager with names `summer_portal_tier_1` through
`summer_portal_tier_5`, `sovereigns_portal`, `sky_ruins_portal_tier_1` through
`sky_ruins_portal_tier_5`, and `lightning_gods_portal`.
The reward-card step is gated by the `portal_reward_screen` image, which
prevents battle HUD text from triggering Portal Card Click. If that screen
crop does not match your Roblox layout, capture a replacement or extra
variant with **Settings > General > Image Manager** using the exact name
`portal_reward_screen`.
Portal Mode follows `Events -> Tidal Siege -> Event Gamemode -> Portal Mode`,
then searches the portal inventory for the selected portal image and clicks
`Activate Portal`.
After a Victory, a repeated Portal task clicks `Select Portal`, chooses the
configured portal tier again, and activates it for the next run. A Defeat uses
`Repeat Stage`; three consecutive defeats consume the portal's three hearts and
stop that task.
...
The Summer task follows this in-game flow:

`Events -> Tidal Siege -> Event Gamemode`

From there choose one of two modes:

- **Event Mode** -> Select Stage -> Start.
- **Portal Mode** -> choose the portal image from the already-open inventory
  -> Activate Portal -> Start.

The matching reference crops live under the Summer-named folders in
`Assets/ui/`, including `summer_tidal_siege/`, `summer_event_mode/`,
`summer_portal_mode/`, the tier-specific portal folders, and
`activate_portal/`.
