...
function taskSummary(t) {
  ...
  if (t.mode === 'story' || t.mode === 'raid') {
    ...
    if (summerMode === 'Portal Mode') {
      ...
      const tierLabel = tier === 'Secret'
        ? (portal === 'Summer Portal' ? "Sovereign's Portal" : "Lightning God's Portal")
        : `Tier ${tier}`;
      title += ` · ${portal} · ${tierLabel}`;
    }
  }
...
const BLOCK_TYPES = {
  ...
  click_auto_play:    { label: 'Auto Play',          group: 'Units',  color: 'var(--rose)',  params: [] },
...
const PHASE_ALLOWED = {
  ...
  prestart: ['place_unit', 'setting_change', 'auto_upgrade_unit', 'target_priority', 'click_auto_play', 'walk', 'record', 'click', 'wait_ms', 'send_key', 'detect'],
