...
class BlockOps:
    ...
    def _run_battle_blocks_tick(self, hwnd, stop_event: threading.Event, battle_blocks: list, first_repeat: bool,
        ...
        while self._battle_block_index < len(battle_blocks):
            ...
            if btype == "upgrade_unit":
                ...
                self._battle_block_state = {}
            elif btype == "click_auto_play":
                done = self._run_click_auto_play_tick(
                    hwnd, stop_event, self._battle_block_index + 1)
                self._battle_block_state = {}
            elif btype == "send_key":
    ...
    def _run_click_auto_play_tick(self, hwnd, stop_event: threading.Event, block_num: int) -> bool:
        """Find and click the in-game Auto-Play button once.

        Returning False when the button is not visible keeps the block active
        until the button appears; this is useful when the match UI animates in.
        Put the block in a Loop phase if it should keep checking without
        delaying other one-shot Battle blocks.
        """
        label = f"Battle block #{block_num} (Auto Play)"
        self._set_status(action="Looking for Auto-Play...")
        match = self._click_found_image(
            hwnd, AUTO_PLAY_IMAGE, AUTO_PLAY_WAIT_TIMEOUT, stop_event)
        if match is None:
            return stop_event.is_set()
        self._log(f"{label}: Auto-Play clicked.")
        return True
    ...
    def _run_prestart_single_block(self, hwnd, stop_event: threading.Event, task: dict, default_walk_paths: dict,
        ...
        if btype == "place_unit":
            ...
            self._run_click_auto_play_tick(hwnd, stop_event, i)
        elif btype == "wait_ms":
