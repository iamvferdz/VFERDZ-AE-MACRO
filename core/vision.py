...
def find_color_image(hwnd: int, name: str, region: tuple = None, threshold: float = 0.90,
                     template_dir: str = UI_ASSETS_DIR) -> dict:
    """Match a UI image while preserving its color information.

    This is important for portal tiers: their shape and label are identical,
    but the tier border/background color is deliberately different.
    """
    paths = template_variant_paths(name, template_dir)
    if not paths:
        raise TemplateNotFound(f'No reference image for "{name}"')
    haystack = capture_game_bgr(hwnd, region)
    if haystack is None:
        return None
    best = None
    for path in paths:
        ...
        for scale in SCALE_FACTORS:
            ...
            if scale != 1.0:
                ...
                candidate = cv2.resize(
                    template, (max(1, round(w * scale)), max(1, round(h * scale))),
...
def wait_for_color_image(hwnd: int, name: str, region: tuple = None, threshold: float = 0.90,
                         timeout: float = 8.0, interval: float = 0.3,
                         stop_event: threading.Event = None, template_dir: str = UI_ASSETS_DIR) -> dict:
    deadline = time.time() + timeout
    while time.time() < deadline:
        if stop_event is not None and stop_event.is_set():
            return None
        match = find_color_image(hwnd, name, region, threshold, template_dir)
        if match is not None:
            return match
        time.sleep(interval)
    return None
...
def find_image_all(hwnd: int, name: str, region: tuple = None, threshold: float = DEFAULT_THRESHOLD,
    ...
    threshold = _effective_threshold(name, threshold)
    kept = []
    # Cards can render substantially larger in the battle result overlay than
    # their inventory reference crop. Scan the same supported scales used by
    # single-image matching and suppress duplicate detections afterward.
    for scale in SCALE_FACTORS:
        for template_gray, mask in _scaled_templates(name, template_dir, scale):
            ...
            result = cv2.matchTemplate(
                haystack, template_gray,
                cv2.TM_CCORR_NORMED if mask is not None else cv2.TM_CCOEFF_NORMED,
                mask=mask,
            )
            result[~np.isfinite(result)] = -1
            ys, xs = np.where(result >= threshold)
            for y, x in zip(ys, xs):
                candidate = {
                    "_rx": int(x), "_ry": int(y), "x": int(x), "y": int(y),
                    "w": tw, "h": th, "cx": int(x + tw // 2),
                    "cy": int(y + th // 2), "score": float(result[y, x]),
                }
                if not any(
                    (candidate["_rx"] - prior["_rx"]) ** 2
                    + (candidate["_ry"] - prior["_ry"]) ** 2
                    < (max(tw, th) * 0.5) ** 2
                    for prior in kept
                ):
