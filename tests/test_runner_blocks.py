...
def test_click_auto_play_waits_until_image_is_found():
    runner = DummyRunner()
    runner._set_status = lambda **_kwargs: None
    runner._click_found_image = MagicMock(side_effect=[None, {"score": 0.95}])
    stop_event = __import__("threading").Event()

    assert runner._run_click_auto_play_tick(123, stop_event, 4) is False
    assert runner._run_click_auto_play_tick(123, stop_event, 4) is True
    runner._click_found_image.assert_called_with(
        123, "auto_play", 1.0, stop_event)
