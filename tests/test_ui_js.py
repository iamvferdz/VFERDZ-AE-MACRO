...
def test_summer_portal_task_summary_shows_tier(tmp_path):
    out = run_js("""
        const TASK_DATA = { summer: { label: 'Summer' } };
        const DEFAULT_INFINITE_WAVE_LIMIT = 20;
        eval(extract('taskSummary'));
        console.log(JSON.stringify(taskSummary({
          mode: 'summer', summer_mode: 'Portal Mode', map: 'Summer Portal',
          portal_tier: '3', repeat: 1, play_mode: 'solo', macro: ''
        })));
    """, tmp_path)
    assert "Summer Portal" in out["title"]
